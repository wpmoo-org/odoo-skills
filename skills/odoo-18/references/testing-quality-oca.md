# Testing, Quality, And OCA Review For Odoo 18.0
...
- For Odoo 18.0, HOOT-based JS tests live under `/static/tests`, usually end in
  `.test.js`, are included in `web.assets_unit_tests`, and can be run through
  `/web/tests`.
...
- Use `pylint-odoo --valid-odoo-versions=18.0` where the project exposes a
...
- Manifest version starts with `18.0.`.
