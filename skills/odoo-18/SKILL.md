---
name: odoo-18
description: Use when creating, editing, reviewing, testing, or porting Odoo 18.0 addons with official Odoo 18 developer APIs and OCA-compatible conventions.
---

# Odoo 18.0 Addon Development

Use this skill for Odoo 18.0 work. Keep the target branch, manifest version,
runtime commands, and source links pinned to 18.0 unless the user explicitly
asks for a cross-version port.

## First steps

1. Confirm the addon repository branch is `18.0` or that the task explicitly
   targets Odoo 18.0.
2. Inspect the addon manifest and touched files before editing.
3. Prefer official Odoo 18.0 documentation and the `odoo/odoo` `18.0` branch
...
- `references/porting-upgrade.md`: upgrade scripts, upgrade utils, forward-port
  rules, 18.0-specific compatibility checks.

## Odoo 18.0 defaults

- Manifest versions must start with `18.0`, for example `18.0.1.0.0`.
...
- SQL constraints use `_sql_constraints`; Python business constraints use
  `@api.constrains`.
...
- Follow OCA manifest versioning: `18.0.x.y.z`.
...
```bash
node skills/odoo-18/scripts/validate-addon.mjs /path/to/addons-or-addon
```

This is a fast heuristic check only. It does not replace installing/updating the
addon in an Odoo 18.0 database.
...
- Views use Odoo 18.0 syntax and avoid legacy modifiers.
- Python, XML, JS, and translation changes pass configured linters.
- The addon installs or updates cleanly in an 18.0 database with relevant tests.
