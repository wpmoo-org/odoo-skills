...
async function main() {
  ...
  for (const file of files) {
    ...
    if (file.endsWith(".xml")) {
      if (/<list(\s|>)/.test(text)) {
        report("ERROR", file, "Odoo 17.0 list views use <tree>, not <list>.");
        failures += 1;
      }
      if (/<field[^>]+name=["']view_mode["'][^>]*>[^<]*\blist\b[^<]*<\/field>/m.test(text) || /\bview_mode=["'][^"']*\blist\b[^"']*["']/.test(text)) {
        report("ERROR", file, "Odoo 17.0 action view_mode values use tree, not list.");
        failures += 1;
      }
