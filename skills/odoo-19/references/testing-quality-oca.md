# Testing, Quality, And OCA Review For Odoo 19.0
...
- For module update workflows, Odoo 19.0 CLI supports upgrading only outdated
  modules through `odoo-bin module upgrade --outdated`.
...
odoo-bin -d test_db --addons-path=/path/to/addons module upgrade addon_name
```
...
- Use `pylint-odoo --valid-odoo-versions=19.0` where the project exposes a
...
- Manifest version starts with `19.0.`.
