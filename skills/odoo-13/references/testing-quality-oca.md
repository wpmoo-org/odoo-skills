...
- Test access errors, constraints, onchange behavior, computed fields,
  multi-company behavior, and migration-sensitive data when relevant.

## JavaScript Tests

- Use Odoo 13's legacy JavaScript test tooling and asset setup.
- Keep test code under the repository's established `static/tests` or
  `static/src` convention.
- Do not introduce native-module or HOOT-only APIs.

## OCA Quality Gates

- Run the repository pre-commit hooks.
- Use `pylint-odoo --valid-odoo-versions=13.0` when linting standalone.
...
- Manifest version starts with `13.0.`.
...
- Assets use legacy Odoo 13 bundle inheritance.
